...
class GlassAgentAuthenticationTests(unittest.TestCase):
    def test_status_socket_url_never_contains_the_permanent_key(self):
        url = agent_config.ws_url("https://glass.example")
    ...
    def test_both_supported_websocket_exception_shapes_detect_auth_rejection(self):
        ...
        self.assertTrue(agent_config.is_authentication_rejection(direct))

        nested = RuntimeError("rejected")
        nested.response = type("Response", (), {"status_code": 401})()
        self.assertTrue(agent_config.is_authentication_rejection(nested))
        self.assertFalse(agent_config.is_authentication_rejection(RuntimeError("offline")))

    def test_configured_key_spellings_are_trimmed_consistently(self):
        self.assertEqual(
            agent_config.api_key_from_config({"api_key": "  primary-key \n"}),
            "primary-key",
        )
        self.assertEqual(
            agent_config.api_key_from_config(
