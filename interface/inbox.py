"""The durable inbox: reading it, committing it, and waking on it.

The CLI writes complete frames to a JSONL file and Stemcell commits its byte
offset only after a whole frame has been interpreted. That is what makes a
crash mid-turn safe: the frame is read again rather than lost. A listener
thread keeps the read live so a long manager turn never stalls ingestion.
"""
