...
### WSL Windows-executable registration lost after Ubuntu upgrades

The Windows x64 [verification run](https://github.com/teamofsilicons/silicon-stemcell/actions/runs/35139664235) imported the checksum-pinned Ubuntu 24.04 image, upgraded systemd from `255.4-1ubuntu8` to `255.4-1ubuntu8.17`, and reloaded the dedicated distribution. The full interpreter E2E passed, but `silicon web` failed to execute Windows PowerShell with `Exec format error`. This matches the registration-loss mechanism described in [Canonical's binfmt documentation](https://ubuntu.com/wsl/docs/stable/explanation/binfmt/) and [Microsoft's WSL interoperability architecture](https://wsl.dev/technical-documentation/interop/).

The Windows installer now probes real Windows process execution after provisioning. If that fails, it restores only a missing WSL interoperability registration using [Microsoft's exact WSL2 `FP` registration](https://github.com/microsoft/WSL/blob/40f18621d54abc1472657c07cc9fbb50cb9f2351/src/linux/init/binfmt.h), then probes again. It refuses to replace an existing registration or enable globally disabled binfmt, and it does not shut down other WSL distributions. A persistent distribution configuration preserves the registration for future service starts. The native integration test additionally checks Windows PowerShell output and exit status after the distribution reload. Final hosted verification is recorded in the implementation diary; the patch does not change WSL or Ubuntu source.
