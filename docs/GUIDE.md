...
| 4.0.0 Honeycomb package | The actual six-target archive passed Honeycomb 0.2.3 directory and archive validation: 348,665,333 compressed bytes, 705,208,541 expanded bytes, 229 entries. It exports `silicon` and `si`; both Windows runtimes match the verified Linux archives. Catalog registration, upload, private installation, and public review remain blocked by mandatory IAM permissions/webhook requirements, as recorded in the external issue ledger. |
