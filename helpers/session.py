...
This lives in ``helpers/`` rather than in ``silicon/`` on purpose: ``interface/``
needs it to resolve a room, and ``silicon/`` imports ``interface/`` heavily, so
putting it in ``silicon/`` would close a package cycle. It is the leaf both sides
depend on.
